...
Again you can see the analog_in.py taking up most of the program time for the RPi0.  Similar analysis was done for the RPi3A+.
...
By analyzing the results you could see why a microcontroller like the ESP32 can sometimes be a better option for an analog application where you're reading a variable voltage. (the joystick reads the movement and outputs it as a voltage)

And as noticed with the choppy buggy movement the Pi0 takes the longest, >50ms, to process/read the joystick thru the MCP3008. The ESP32 collects the joystick response the fastest at <1ms.  ​The Pi3A+ was close but still slower than the ESP32 at 2.8ms.